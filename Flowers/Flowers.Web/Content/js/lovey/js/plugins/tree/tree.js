...
(function ($) {
    ...
    $.tree.deleteNode = function (treeId, url, execFunc) {
        ...
        if (ids == "") {
            $.ligerDialog.alert("请选择要删除的数据！", "提示", "warn");
            return;
        }
        $.ligerDialog.confirm("确定要删除“" + names + "”吗？", function (r) {
            if (r) {
                $.ajax({
                    type: 'post',
                    url: url ? url : '',
        ...
        this.each(function () {
            ...
            var g = {
                ...
                hasChildren: function (treenodedata) {
                    if (typeof (p.isLeaf) == "function") return p.isLeaf(treenodedata);
                    //by yzhao 带智能判断，如果判断节点是否是子节点未给出也可以实现
                    return (treenodedata.children && treenodedata.children != "" && treenodedata.children.length > 0) || (treenodedata.leaf != undefined && !treenodedata.leaf) ? true : false;
                },
                //获取父节点
            ...
            var po = {
                ...
                getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand) {
                    ...
                    if (data != undefined) {
                        for (var i = 0; i < data.length; i++) {
                            ...
                            //增加属性支持
                            for (var l = 0; l < g.sysAttribute.length; l++) {
                                if ($(this).attr(g.sysAttribute[l]))
                                    data[dataindex][g.sysAttribute[l]] = $(this).attr(g.sysAttribute[l]);
                            }
                            ...
                            for (var kk = 0; kk <= outlineLevel - 2; kk++) {
                                if (isLast[kk]) treehtmlarr.push('<div class="l-box"></div>');
                ...
                getDataByTreeHTML: function (treeDom) {
                    ...
                    $("> li", treeDom).each(function (i, item) {
                        ...
                        for (var k = 0; k < p.attribute.length; k++) {
                            if ($(this).attr(p.attribute[k]))
                                data[dataindex][p.attribute[k]] = $(this).attr(p.attribute[k]);
                        }
                        if ($("> ul", this).length > 0) {
                ...
                setTreeEven: function () {
                    ...
                    g.tree.click(function (e) {
                        ...
                        if (obj.tagName.toLowerCase() == "a" || obj.tagName.toLowerCase() == "span" || $(obj).hasClass("l-box"))
                            ...
                            treeitem = $(obj);
                        if (!treeitem) return;
                        var folderitem = $(obj).next();//获取+-号所在元素的后辈元素
                        var treedataindex = parseInt(treeitem.attr("treedataindex"));
                        var treenodedata = po.getDataNodeByTreeDataIndex(g.data, treedataindex);
                        ...
                        if ($(obj).hasClass("l-checkbox") && !$(obj).siblings().hasClass("gray")) {
                            ...
                            if (!(obj.tagName.toLowerCase() == 'span')) {
                                ...
                                //单选多选操作的元素不一致，需要注意
                                if (p.checkbox) {
                                    folderitem.next().removeClass("l-tree-icon-folder-open").addClass("l-tree-icon-folder");
                                } else {
                                    folderitem.removeClass("l-tree-icon-folder-open").addClass("l-tree-icon-folder");
                                }
                                if (p.slide)
                            ...
                            if (!(obj.tagName.toLowerCase() == 'span')) {
                                ...
                                if (p.checkbox) {
                                    folderitem.next().removeClass("l-tree-icon-folder").addClass("l-tree-icon-folder-open");
                                } else {
                                    folderitem.removeClass("l-tree-icon-folder").addClass("l-tree-icon-folder-open");
                                }
